...
void MeshPainter::drawMesh(TriMesh* mesh, openGLObject& object, Light* light, Camera* camera) {
	...
	if (!mesh->isRobot)
	{
		mesh->modelMatrix = mesh->getModelMatrix();
	}

	// 传递矩阵
	glUniformMatrix4fv(object.modelLocation, 1, GL_FALSE, &mesh->modelMatrix[0][0]);
	...
	if (!mesh->isPlane)
	{
		...
		glm::mat4 shadowModelMatrix = shadowProjMatrix * mesh->modelMatrix;
